...
use crate::key_seal::{EcPublicEncryptionKey, ec_public_key::EcPublicKey, ec_key::EcKey};
...
impl PrivateKey for EcEncryptionKey {
    ...
    async fn import(pem_bytes: &[u8]) -> Result<Self, TombCryptError> {
        Ok(Self(EcKey(import_key_pem(pem_bytes)?)))
    }

    async fn import_bytes(der_bytes: &[u8]) -> Result<Self, TombCryptError> {
        Ok(Self(EcKey(import_key_bytes(der_bytes)?)))
    }
}
